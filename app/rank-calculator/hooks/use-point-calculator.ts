...
export function usePointCalculator() {
  ...
  const joinDate = useWatch<FormData, 'joinDate'>({
    name: 'joinDate',
  });
  const items = useWatch<FormData, 'items'>({
    name: 'items',
  });
  const ehp = useWatch<FormData, 'ehp'>({
    name: 'ehp',
  });
  const ehb = useWatch<FormData, 'ehb'>({
    name: 'ehb',
  });
  const achievementDiaries = useWatch<FormData, 'achievementDiaries'>({
    name: 'achievementDiaries',
  });
  const caTier = useWatch<FormData, 'caTier'>({
    name: 'caTier',
  });
